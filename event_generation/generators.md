...
The output file `z_mumu.hepmc` from generating events with Whizard is in the `hepMC` format. We can convert this file to the preferred `EDM4hep` format by using a Gaudi steering file. Let's download a minimal example of such file:
...
This can be run as the following:

```bash
cd ..
k4run configs/hepmc2edm.py -n 100 --GenAlg.HepMCFileReader.Filename Whizard/Z_mumu/z_mumu.hepmc --out.filename Whizard/Z_mumu/z_mumu.e4h.root
```
